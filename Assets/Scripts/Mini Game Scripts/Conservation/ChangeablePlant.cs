...
public class ChangeablePlant : MonoBehaviour
{
    ...
    [Header("UI Panels")]
    [SerializeField]
    public GameObject correctPlantSwappedPanel;

    [SerializeField]
    public GameObject incorrectPlantSwappedPanel;
            ...
            if (originalPlantIndex >= 0) 
            {
                ...
                if (originalPlantIndex == 0)
                {
                    ...
                    if (plantComponent.plantID == "Bradford Pear Tree 1")
                    {
                        ...
                        animalGameManager.PlayCorrectPlantSwappedSound(); // Play the correct plant selected sound
                ...
                else if (originalPlantIndex == 6)
                {
                    ...
                    if (plantComponent.plantID == "Purple Loosestrife 1")
                    {
                        ...
                        if (animalGameManager.wasReplaceWithAmericanLotusButtonClicked)
                        {
                            ...
                            {
                                IncorrectPlantSwapped(); // Show the incorrect plant swapped panel
                            }

                            animalGameManager.PurpleLoosestrifesSwapped += 1; // Increment the number of Purple Loosestrifes swapped
                            animalGameManager.PlayIncorrectPlantSwappedSound(); // Play the incorrect plant selected sound
                        ...
                        // If the replace with Yellow Coneflower button as clicked...
                        else if (animalGameManager.wasReplaceWithYellowConeflowerButtonClicked)
                        {
                            ...
                            {
                                CorrectPlantSwapped(); // Show the correct plant swapped panel
                            }
    ...
    private void IncorrectPlantSwapped()
    {
        ...
        //Debug.Log($"incorrectPlantSwappedPanelShown: {incorrectPlantSwappedPanelShown}");
        playerInput.controlsLocked = true; // Unlock player controls when the panel is clicked

        // Reset the typewriter effect
        var typewriter = correctPlantSwappedPanel.GetComponentInChildren<TypewriterUi>();
        if (typewriter != null)
            typewriter.RestartTypewriter();
    }

    private void CorrectPlantSwapped()
    {
        ...
        //Debug.Log($"correctPlantSwappedPanelShown: {correctPlantSwappedPanelShown}");
        playerInput.controlsLocked = true; // Unlock player controls when the panel is clicked
    ...
    public void ResetIncorrectPlantPanelState()
    {
        //Debug.Log("Resetting Incorrect Plant Panel..."); // Debug.Log
        incorrectPlantSwappedPanelShown = false; // Reset the flag to indicate that the incorrect plant swapped panel is not shown
        incorrectPlantSwappedPanel.SetActive(false); // Hide the incorrect plant swapped panel
        playerInput.controlsLocked = false; // Lock player controls when the panel is clicked
    }

    public void ResetCorrectPlantPanelState()
    {
        //Debug.Log("Resetting Correct Plant Panel..."); // Debug.Log
        correctPlantSwappedPanelShown = false; // Reset the flag to indicate that the correct plant swapped panel is not shown
        correctPlantSwappedPanel.SetActive(false); // Hide the correct plant swapped panel
        playerInput.controlsLocked = false; // Lock player controls when the panel is clicked
    }

    public static void TryClosePlantSwappedPanelsOnE()
    {
        foreach (var plant in Object.FindObjectsByType<ChangeablePlant>(FindObjectsSortMode.None))
        {
            //Debug.Log($"correctPlantSwappedPanelShown: {correctPlantSwappedPanelShown}, incorrectPlantSwappedPanelShown: {incorrectPlantSwappedPanelShown}");
            if (correctPlantSwappedPanelShown && plant.correctPlantSwappedPanel != null && plant.correctPlantSwappedPanel.activeSelf)
            {
                //Debug.Log($"{plant.plantID} - Closing correct panel");
                plant.ResetCorrectPlantPanelState();
            }
            if (incorrectPlantSwappedPanelShown && plant.incorrectPlantSwappedPanel != null && plant.incorrectPlantSwappedPanel.activeSelf)
            {
                //Debug.Log($"{plant.plantID} - Closing incorrect panel");
                plant.ResetIncorrectPlantPanelState();
            }
        }
    }
}
